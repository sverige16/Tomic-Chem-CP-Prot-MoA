...
from Erik_helper_functions import  conf_matrix_and_class_report, program_elapsed_time, create_splits, extract_all_cell_lines
...
cnn_mod = CNN_Model(num_features = 978, num_targets = 15, hidden_size= hidden_size)
cnn_mod.to(device)

class CNN_and_Cell_Line_Model(nn.Module):
    def __init__(self, cnn_model, cell_line_model, num_targets = None):
        super(CNN_and_Cell_Line_Model, self).__init__()
        self.SimpleNN = cnn_model
...
model = CNN_and_Cell_Line_Model(cnn_mod, cell_line_mod, num_targets = 10)
...
model_test = CNN_and_Cell_Line_Model(cnn_mod, cell_line_mod, num_targets = 10)
best_model = torch.load('/home/jovyan/Tomics-CP-Chem-MoA/04_Tomics_Models/Best_Tomics_Model/saved_models' +'/' + 'Tomics_1DCNN')
model_test.load_state_dict(best_model['model_state_dict'])
...
val_vs_train_loss(num_epochs,train_loss_per_epoch, val_loss_per_epoch, now, '1DCNN', '/home/jovyan/Tomics-CP-Chem-MoA/04_Tomics_Models/Best_Tomics_Model/saved_images') 
val_vs_train_accuracy(num_epochs, train_acc_per_epoch, val_acc_per_epoch, now,  '1DCNN', '/home/jovyan/Tomics-CP-Chem-MoA/04_Tomics_Models/Best_Tomics_Model/saved_images')
...
conf_matrix_and_class_report(all_labels, all_predictions, '1DCNN')
