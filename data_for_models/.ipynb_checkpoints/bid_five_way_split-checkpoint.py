...
def check_veracity_cmpd_num(file, X):
    if file == 'tian10':
        ...
        assert X.groupby(['Compound_ID', 'Batch_nr']).ngroups == 76, 'Incorrect number of unique compounds'
    else:
...
def main(subset, file_tosplit, compounds_v1v2, str_help, hq = "False", dosage = "False", low_t = 0, high_t = 100):
    ...
    # We want to split the data based on the moa, but only for unique compounds.
        compounds_for_splitting = compounds_v1v2[["Compound_ID", "Batch_nr", "moa"]].drop_duplicates()
    ...
    if subset == "cyc_adr":
        ...
        X = compounds_for_splitting["Compound_ID", "Batch_nr"]
        ...
        assert set(compounds_for_splitting.moa) == set(moas), "There might be multple moas (cyclo | adr) in the data. "
        X = compounds_for_splitting["Compound_ID", "Batch_nr"]
        X = X.set_index(["Compound_ID", "Batch_nr"])
        y = compounds_for_splitting["moa"]
    elif subset == "0":
    ...
    for i, (train_val_index, test_index) in enumerate(skf.split(X,y)):
        ...
        train = compounds_v1v2[compounds_v1v2["Compound_ID", "Batch_nr"].isin(list(X.iloc[train_index]))].reset_index(drop = True)
        test = compounds_v1v2[compounds_v1v2["Compound_ID", "Batch_nr"].isin(list(X.iloc[test_index]))].reset_index(drop = True)
        val = compounds_v1v2[compounds_v1v2["Compound_ID", "Batch_nr"].isin(list(X.iloc[val_index]))].reset_index(drop = True)
        ...
        inter1 = set(list(train["Compound_ID", "Batch_nr"])) & set(list(val["Compound_ID", "Batch_nr"]))
        inter2 = set(list(train["Compound_ID", "Batch_nr"])) & set(list(test["Compound_ID", "Batch_nr"]))
        inter3 = set(list(val["Compound_ID", "Batch_nr"])) & set(list(test["Compound_ID", "Batch_nr"]))
