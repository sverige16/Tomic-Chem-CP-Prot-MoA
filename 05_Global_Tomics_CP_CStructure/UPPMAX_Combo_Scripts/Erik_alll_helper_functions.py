...
def conf_matrix_and_class_report(labels_val, predictions, model_name, dict_moa = None):
    ...
    plt.savefig('/home/jovyan/saved_class_info_matrix/conf_matrix.png')
   
    class_report = classification_report(labels_val, predictions, target_names= list_of_MoA_names)
    print(class_report)
    f = open("/home/jovyan/saved_class_info_matrix/class_info.txt","w")
    # write file
...
def set_parameter_requires_grad(model, feature_extracting, added_layers = 0):
    ...
    if feature_extracting:
        ...
        for name, param in model.named_parameters():
            if added_layers == 1:
                if name.startswith('linear_layer_pre') or name.startswith('additional_layers'):
                    param.requires_grad = True
            if added_layers == 0:
                if name.startswith('additional_layers'):
                    param.requires_grad = True
    else:
...
def tprofiles_gc_too_func(data, clue_gene):
    ...
    tprofiles_gctoo = parse("/home/jovyan/data_for_models/level5_beta_trt_cp_n720216x12328.gctx", 
                                    cid= profile_ids, 
...
def create_splits(train, val, test, hq = "False", dose = "False", cell_lines = []):
    ...
    # read in documents
    clue_sig_in_SPECS = pd.read_csv('/home/jovyan/data_for_models/clue_sig_in_SPECS1&2.csv', delimiter = ",")
...
def accessing_all_folds_csv(file, fold_int):
    ...
    if file == 'tian10':
        ...
        dir_path = '/home/jovyan/data_for_models/5_fold_data_sets/erik10/'
        if fold_int ==0:
...
def upload_to_neptune(neptune_project_name,
    ...
    if val_vs_train_acc_path or val_vs_train_acc_path:
         # Upload model
        run["optimal_model_checkpoint"].upload('/home/jovyan/saved_models/' + model_name + ".pt")
         # Uploading validation metrics
        state = torch.load('/home/jovyan/saved_models/' + model_name + ".pt")
    ...
    run["files/classif_info"].upload('/home/jovyan/saved_class_info_matrix/class_info.txt')
    run["images/conf_matrix"].upload('/home/jovyan/saved_class_info_matrix/conf_matrix.png')
    run.stop()
...
def adapt_training_loop(n_epochs,
    ...
    '''
    Can handle one, two or three different inputs from the various types of data.
    Inputs:
        ...
        loss_fn_str: string, loss function used to train the model
        train_loader: torch.utils.data.DataLoader, training data loader
        valid_loader: torch.utils.data.DataLoader, validation data loader
        device: torch.device, device to train the model on
        model_name: string, name of the model
        val_str: string, fe, f1, or loss
        early_patience: int, number of epochs to wait before stopping training if validation loss does not improve
        loss_fn_train: string, loss function used to train the model
        my_lr_scheduler: string, learning rate scheduler used to train the model
    Outputs:
        train_loss_per_epoch: list, training loss per epoch
        train_acc_per_epoch: list, training accuracy per epoch
        val_loss_per_epoch: list, validation loss per epoch
        val_acc_per_epoch: list, validation accuracy per epoch
        val_f1_score_per_epoch: list, validation f1 score per epoch
    ...
    for epoch in tqdm(range(1, n_epochs +1), desc = "Epoch", position=0, leave= False):
        ...
        for data_labels in tqdm(train_loader, desc = "train batch", position=0, leave= False):
            ...
            if torch.isnan(loss) or loss > 10000:
                print("Loss is NaN or too large. Stopping training.")
    ...
    def validation_saving(val_str, 
        ...
        if val_str == "fe":
            ...
            if val_metric < avg_f1:
                ...
                },  '/home/jovyan/saved_models/' + model_name + ".pt"
                )
...
def adapt_test_loop(model, 
    ...
    Output:
        ...
        loss_test: float, average loss over the test set
        all_predictions: list, all predictions from the model
        all_labels: list, all labels from the model
    '''
    model = model.to(device)
    model.eval()
    loss_test = 0.0
...
def adapt_test_loop_soft_vote2(model, 
                    test_loader, 
                    device):
    '''
    Test loop for testing soft voting
    Input:
        model: model to be tested
        test_loader: test data loader
        device: device to run the model on
    Output:
        all_predict_proba_1: list, all predictions from the model1
        all_predict_proba_2: list, all predictions from the model2
        all_labels: list, all labels from the model
    '''
    model = model.to(device)
    model.eval()
    ...
    with torch.no_grad():  # does not keep track of gradients so as to not train on test data.
        for data_labels in tqdm(test_loader, desc = "test", position=0, leave= False):
            ...
            all_predict_proba_1 = all_predict_proba_1 + torch.nn.functional.softmax(outputs[0]).cpu().numpy().tolist()
            all_predict_proba_2  = all_predict_proba_2 + torch.nn.functional.softmax(outputs[1]).cpu().numpy().tolist()
            all_labels = all_labels + torch.max(labels, 1)[1].tolist()
    return  all_predict_proba_1, all_predict_proba_2, all_labels

def adapt_test_loop_soft_vote3(model, 
                    test_loader, 
                    device):
    '''
    Input:
    ...
    Output:
        all_predict_proba_1: list, all predictions from the model1
        all_predict_proba_2: list, all predictions from the model2
        all_predict_proba_3: list, all predictions from the model3
        all_labels: list, all labels from the model
    '''
    model = model.to(device)
    model.eval()
    all_labels = []
    all_predict_proba_CP = []
    all_predict_proba_CS = []
    all_predict_proba_GE = []
    with torch.no_grad():  # does not keep track of gradients so as to not train on test data.
        for data_labels in tqdm(test_loader, desc = "test", position=0, leave= False):
            # put model, images, labels on the same device
            data = [d.to(device=device) for d in data_labels[:-1]]  # Excluding the labels from data
            ...
            outputs = model(*data)
            # Convert to numpy array and then to list
            all_predict_proba_CP = all_predict_proba_CP + torch.nn.functional.softmax(outputs[0]).cpu().numpy().tolist()
            all_predict_proba_CS  = all_predict_proba_CS + torch.nn.functional.softmax(outputs[1]).cpu().numpy().tolist()
            all_predict_proba_GE = all_predict_proba_GE + torch.nn.functional.softmax(outputs[2]).cpu().numpy().tolist()
            all_labels = all_labels + torch.max(labels, 1)[1].tolist()
    return  all_predict_proba_CP, all_predict_proba_CS, all_predict_proba_GE, all_labels

def ensemble_predict2(test_generator, model, weights, device):
    '''
    Conducts soft voting on the outputs of two models
    '''
    all_predict_proba_model1, all_predict_proba_model2, all_labels = adapt_test_loop_soft_vote2(model = model, 
                    test_loader = test_generator, 
                    device = device)
    # Compute weighted average of outputs
    out_ensemble = []
    for i in range(len(all_predict_proba_model1)):
        mod1 = all_predict_proba_model1[i]
        mod2 = all_predict_proba_model2[i]
        mod1_w = [x * weights[0] for x in mod1]
        mod2_w = [x * weights[1] for x in mod2]
        sum_list = [x + y for x, y in zip(mod1_w, mod2_w)]
        out_ensemble.append(max(enumerate(sum_list), key=lambda x: x[1])[0])
    return out_ensemble, all_labels

def ensemble_predict3(test_generator, model, weights, device):
    '''
    Conducts soft voting on the outputs of three models
    '''
    all_predict_proba_model1, all_predict_proba_model2, all_predict_proba_model3, all_labels = adapt_test_loop_soft_vote3(model = model, 
                    test_loader = test_generator, 
                    device = device)
    # Compute weighted average of outputs
    out_ensemble = []
    for i in range(len(all_predict_proba_model1)):
        mod1 = all_predict_proba_model1[i]
        mod2 = all_predict_proba_model2[i]
        mod3 = all_predict_proba_model3[i]
        mod1_w = [x * weights[0] for x in mod1]
        mod2_w = [x * weights[1] for x in mod2]
        mod3_w = [x * weights[2] for x in mod3]
        sum_list = [x + y + z for x, y, z in zip(mod1_w, mod2_w, mod3_w)]
        out_ensemble.append(max(enumerate(sum_list), key=lambda x: x[1])[0])
    return out_ensemble, all_labels
############################################################################################################
...
def CP_driving_code(file_name, fold_int):
    ...
    Output:
        ...
        validation_df: dataframe, validation data
        test_df: dataframe, test data
        dict_moa: dictionary, dictionary which associates moa with a one hot encoded number
        training_set: dataset, training set with compounds
        validation_set: dataset, validation set with compounds
        test_set: dataset, test set with compounds
        images: None given hp5y use
    ...
    # adding moa to meta_data
    L1000_train_moa = L1000_training[["Compound ID", "moa"]].drop_duplicates()
    L1000_val_moa = L1000_validation[["Compound ID", "moa"]].drop_duplicates()
    L1000_test_moa = L1000_test[["Compound ID", "moa"]].drop_duplicates()

    training_df = training_df.merge(L1000_train_moa, left_on = "compound", right_on = "Compound ID")
    validation_df = validation_df.merge(L1000_val_moa, left_on = "compound", right_on = "Compound ID")
    test_df = test_df.merge(L1000_test_moa, left_on = "compound", right_on = "Compound ID")

    return training_df, validation_df, test_df, dict_moa, training_set, validation_set, test_set, images
...
def GE_driving_code(file_name, fold_int):
    ...
    Output:
        train_np: numpy array, training data
        valid_np: numpy array, validation data
        test_np: numpy array, test data
        num_classes: int, number of classes
        dict_cell_lines: dictionary, dictionary which associates cell lines with a one hot encoded number
        num_cell_lines: int, number of cell lines
        dict_moa: dictionary, dictionary which associates moa with a one hot encoded number
        training_set: dataset, training set with compounds
        validation_set: dataset, validation set with compounds
        test_set: dataset, test set with compounds
        L1000_training: dataframe, training data with sig id to tprofiles
        L1000_validation: dataframe, validation data with sig id to tprofiles
        L1000_test: dataframe, test data with sig id to tprofiles

        '''
    # clue row metadata with rows representing transcription levels of specific genes
    clue_gene = pd.read_csv('/home/jovyan/data_for_models/clue_geneinfo_beta.txt', delimiter = "\t")
...
def extracting_pretrained_single_models(single_model_str, fold_num):
    '''
    Extracts pretrained single models
    Input:
        single_model_str: string, either GE, CP, or CS
        fold_num: int, either 0, 1, 2, 3, or 4
    Output:
        pretrained_single_model: pretrained single model'''
    if fold_num == 0:
        if single_model_str == "GE":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/GE_fold0_model_checkpoint_559.pt'
        elif single_model_str == "CP":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/CP_fold0_model_checkpoint_606.pt'
        elif single_model_str == "CS":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/CS_fold0_model_checkpoint_584.pt'
        else:
            ValueError("single_model_str must be either GE, CP, or CS")
    elif fold_num == 1:
        if single_model_str == "GE":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/GE_fold1_model_checkpoint_560.pt'
        elif single_model_str == "CP":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/CP_fold1_model_checkpoint_610.pt'
        elif single_model_str == "CS":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/CS_fold1_model_checkpoint_585.pt'
        else:
            ValueError("single_model_str must be either GE, CP, or CS")
    elif fold_num == 2:
        if single_model_str == "GE":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/GE_fold2_model_checkpoint_561.pt'
        elif single_model_str == "CP":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/CP_fold2_model_checkpoint_611.pt'
        elif single_model_str == "CS":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/CS_fold2_model_checkpoint_586.pt'
        else:
            ValueError("single_model_str must be either GE, CP, or CS")
    elif fold_num == 3:
        if single_model_str == "GE":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/GE_fold3_model_checkpoint_562.pt'
        elif single_model_str == "CP":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/CP_fold3_model_checkpoint_612.pt'
        elif single_model_str == "CS":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/CS_fold3_model_checkpoint_587.pt'
        else:
            ValueError("single_model_str must be either GE, CP, or CS")
    elif fold_num == 4:
        if single_model_str == "GE":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/GE_fold4_model_checkpoint_563.pt'
        elif single_model_str == "CP":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/CP_fold4_model_checkpoint_613.pt'
        elif single_model_str == "CS":
            return '/home/jovyan/data_for_models/Final_Pretrained_Single_Models/CS_fold4_model_checkpoint_588.pt'
        else:
            ValueError("single_model_str must be either GE, CP, or CS")
    else:
