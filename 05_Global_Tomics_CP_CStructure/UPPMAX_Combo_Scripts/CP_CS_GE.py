...
class CS_CP_GE_Model(nn.Module):
    def __init__(self, modelCS, modelCP, modelGE):
        ...
        self.linear_layer_pre = nn.Linear(int(103 + 20 + 1280), 227)
        self.additional_layers = nn.Sequential(
            nn.Linear(in_features=227, out_features=222, bias=True),
            nn.LeakyReLU(negative_slope=0.01),
            nn.BatchNorm1d(222, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
            nn.Dropout(p=0.4231140857690407, inplace=False),
            nn.Linear(in_features=222, out_features=10, bias=True)
        )
...
for i in range(0, 5):
    ...
    training_dataset_CP_GE = CS_CP_GE_Dataset(compound_df = cmpd_training_set, 
    ...
    valid_dataset_CP_GE = CS_CP_GE_Dataset(compound_df =cmpd_validation_set, 
                                            ...
                                            dict_cell_line = dict_cell_lines, 
                                            images_H = images_H)
    test_dataset_CS_CP_GE = CS_CP_GE_Dataset(compound_df = cmpd_test_set, 
    ...
    learning_rate = 0.007053835481861996
    ...
    if yn_class_weights:
        ...
        class_weights = None # choosing loss_function 
    loss_fn_str = 'BCE'
    loss_fn_train, loss_fn = different_loss_functions(loss_fn_str= loss_fn_str, 
                                                    class_weights=class_weights)
    my_lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=3, gamma=0.5390730170455282)
    num_epochs_fe = 12
    ...
    model_test = CS_CP_GE_Model(modelCS, modelCP, modified_GE_model)
