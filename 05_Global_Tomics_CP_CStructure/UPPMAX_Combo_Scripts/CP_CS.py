...
class CS_CP_Model(nn.Module):
    def __init__(self, modelCP, modelCS):
        ...
        self.linear_layer_pre = nn.Linear(int(1280 + 103), 180)
        self.leaky_relu = nn.LeakyReLU(negative_slope=0.01)
        self.batch_norm = nn.BatchNorm1d(180)
        self.additional_layers = nn.Sequential(
            nn.Linear(in_features=180, out_features=10   , bias=True)
                            )
...
for i in range(3,5):
    ...
    learning_rate =    0.022213713273465836
    ...
    my_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max =7)
    num_epochs_fe = 12
