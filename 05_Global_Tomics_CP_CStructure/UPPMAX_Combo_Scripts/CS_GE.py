...
class CS_GE_Model(nn.Module):
    def __init__(self, modelCS, modelGE):
        ...
        self.linear_layer_pre = nn.Linear(int(103 + 20), 119)
        self.leaky_relu = nn.LeakyReLU(negative_slope=0.01)
        self.batch_normpre = nn.BatchNorm1d(119)
        self.additional_layers = nn.Sequential(
            nn.Linear(in_features=119, out_features=246, bias=True),
            nn.LeakyReLU(negative_slope=0.01),
            nn.BatchNorm1d(246, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
            nn.Dropout(p=0.39902124406641964, inplace=False),
            nn.Linear(in_features=246, out_features=10, bias=True)
)
...
for i in range(5):
    ...
    learning_rate = 0.08762301480077071
    ...
    my_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=4)
    num_epochs_fe = 12
