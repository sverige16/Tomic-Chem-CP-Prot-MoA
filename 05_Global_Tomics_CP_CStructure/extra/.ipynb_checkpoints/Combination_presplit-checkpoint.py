...
def func_cell_line_dict(df):
    enc = OneHotEncoder(handle_unknown='ignore')
    enc.fit(df["Cell Line"].unique().reshape(-1,1))          # fit the encoder to the unique values of the moa column
    ...
    return dicti
def func_cell_dose_dict(df):
    enc = OneHotEncoder(handle_unknown='ignore')
    enc.fit(df["Dose"].unique().reshape(-1,1))          # fit the encoder to the unique values of the moa column
    one_hot_encoded = enc.transform(enc.categories_[0].reshape(-1,1)).toarray()
    dicti = {}
    for i in range(0, len(enc.categories_[0])):       # create a dictionary with the one hot encoded values
        dicti[str(enc.categories_[0][i])] = one_hot_encoded[i]
...
dict_cell_line = func_cell_line_dict(training_set)
dict_cell_dose = func_cell_dose_dict(training_set)
