...
class CS_CP_DI_Dataset(torch.utils.data.Dataset):
...
training_dataset_CSCPDI = CS_CP_DI_Dataset(training_set, paths_v1v2, DI_train_tensor, df_train_labels, dict_moa,transform = train_transforms, im_norm= pd_image_norm)
valid_dataset_CSCPDI = CS_CP_DI_Dataset(validation_set, paths_v1v2, DI_val_tensor, df_val_labels, dict_moa, transform = train_transforms, im_norm= pd_image_norm)
test_dataset_CSCPDI = CS_CP_DI_Dataset(test_set, paths_v1v2, DI_test_tensor, df_test_labels, dict_moa, transform = train_transforms, im_norm= pd_image_norm)

# create generator that randomly takes indices from the training set
training_generator = torch.utils.data.DataLoader(training_dataset_CSCPDI, **params)
validation_generator = torch.utils.data.DataLoader(valid_dataset_CSCPDI, **params)
test_generator = torch.utils.data.DataLoader(test_dataset_CSCPDI, **params)

# create a model combining both models
class CS_CP_DI(nn.Module):
    def __init__(self, modelCP, modelDI, modelCS):
        super(CS_CP_DI, self).__init__()
...
def validation_loop(model, loss_fn, valid_loader, best_val_loss, device):
    ...
    with torch.no_grad():  # does not keep track of gradients so as to not train on validation data.
        ...
        if best_val_loss > avg_val_loss:
            ...
            },  '/home/jovyan/Tomics-CP-Chem-MoA/saved_models/' + 'CS_CP_DI_model'
            )
    model.train()
...
def test_loop(model, loss_fn, test_loader, device):
    ...
    with torch.no_grad():  # does not keep track of gradients so as to not train on test data.
        for tprofiles, cmpds, CP_img, labels in tqdm(test_loader,
...
model_test = CS_CP_DI
model_test.load_state_dict(torch.load('/home/jovyan/Tomics-CP-Chem-MoA/saved_models/' + 'CS_CP_DI_model')['model_state_dict'])
...
val_vs_train_loss(num_epochs,train_loss_per_epoch, val_loss_per_epoch, now, 'CS_CP_DI','/home/jovyan/Tomics-CP-Chem-MoA/05_Global_Tomics_CP_CStructure/saved_images') 
val_vs_train_accuracy(num_epochs, train_acc_per_epoch, val_acc_per_epoch, now,  'CS_CP_DI', '/home/jovyan/Tomics-CP-Chem-MoA/05_Global_Tomics_CP_CStructure/saved_images')
...
state = torch.load('/home/jovyan/Tomics-CP-Chem-MoA/saved_models/' + 'CS_CP_DI_model')
...
run["images/accuracy"].upload('/home/jovyan/Tomics-CP-Chem-MoA/05_Global_Tomics_CP_CStructure/saved_images' +'/' + 'acc_train_val_' +'CS_CP_DI' + now + '.png')
