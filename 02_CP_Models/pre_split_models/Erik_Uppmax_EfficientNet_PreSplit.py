...
class UPPMAX_Image_Dataset(torch.utils.data.Dataset):
    def __init__(self, pre_fix_image_df):
    ...
    def __getitem__(self, idx):
        ...
        # ID = self.list_ID[idx]
        image = torch.to_tensor(self.pre_fix_image_df["image"][idx]) # extract image from csv using index
        label = torch.to_tensor(self.pre_fix_image_df["label"][idx])          # extract calssification using index
        return image.float(), label.float()  



paths_v1v2 = pd.read_csv('/scratch2-shared/erikep/uppmax_image.csv')
paths_v1v2['image'] = paths_v1v2['image'].apply(lambda x: np.array(ast.literal_eval(x)))
...
for fold_int in range(0,3):
    ...
    training_dataset = UPPMAX_Image_Dataset(training_df)
    valid_dataset = UPPMAX_Image_Dataset(validation_df)
    test_dataset = UPPMAX_Image_Dataset(test_df)
